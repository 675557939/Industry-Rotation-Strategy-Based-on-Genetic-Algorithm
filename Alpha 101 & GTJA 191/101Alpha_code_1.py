...
def rolling_prod(na):
    """Return the product of values in the array."""
    return np.prod(na)
...
def delta(df, period=1):
    """Difference: today's value minus the value `period` days ago."""
...
def scale(df, k=1):
    """Scale so that sum(abs(df)) = k."""
    return df.mul(k).div(np.abs(df).sum())

def ts_argmax(df, window=10):
    """Index of rolling max (1-based)."""
    return df.rolling(window).apply(np.argmax) + 1 

def ts_argmin(df, window=10):
    """Index of rolling min (1-based)."""
...
def decay_linear(df, period=10):
    ...
    for i in range(period - 1, len(df)):
        ...
        out[i] = (window * w.reshape(-1, 1)).sum(axis=0)
...
def get_alpha(df):
    ...
    stock = Alphas(df_out)
    for name in [
        'alpha001','alpha002','alpha003','alpha004','alpha005','alpha006','alpha007','alpha008',
        'alpha009','alpha010','alpha011','alpha012','alpha013','alpha014','alpha015','alpha016',
        'alpha017','alpha018','alpha019','alpha020','alpha021','alpha022',
        'alpha024','alpha025','alpha026','alpha027','alpha028','alpha029','alpha030',
        'alpha032','alpha033','alpha034','alpha035','alpha036','alpha037','alpha038',
        'alpha040','alpha041','alpha042','alpha043','alpha044','alpha045','alpha046',
        'alpha047','alpha049','alpha050','alpha051','alpha052','alpha053','alpha054',
        'alpha055','alpha060','alpha061','alpha062','alpha064','alpha065','alpha068',
        'alpha074','alpha075','alpha078','alpha081','alpha083','alpha084','alpha085',
        'alpha086','alpha094','alpha095','alpha099','alpha101'
    ]:
        df_out[name] = getattr(stock, name)()
...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.volume = df_data['S_DQ_AMOUNT'].copy()
    
        self.returns = df_data['S_DQ_PCTCHANGE'].copy()
        self.vwap = (df_data['S_DQ_AMOUNT'] * 1000) / (df_data['S_DQ_AMOUNT'] * 100 + 1)
    ...
    def alpha001(self):
        ...
        cond = self.returns < 0
        inner.loc[cond] = stddev(self.returns, 20)
        return rank(ts_argmax(inner ** 2, 5)).fillna(0)
