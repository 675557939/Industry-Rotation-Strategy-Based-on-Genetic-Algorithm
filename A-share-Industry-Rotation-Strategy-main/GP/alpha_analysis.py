...
class FactorAnalysis:
    ...
    def calculate_ic(self):
        ...
        ic_df = pd.DataFrame({'IC': ic.values.astype('float32'), 'p_value': p_value.astype('float32')},
                             index=ic.index)
    ...
    def run_full_analysis(self, n_groups=5):
        ...
        print(f'IC胜率: {(ic_df["IC"] > 0).mean():.2%} | 显著IC胜率: {(ic_df["p_value"] < 0.05).mean():.2%}')
    ...
    @staticmethod
    def _format_date_axis(ax_obj):
        ax_obj.xaxis.set_major_locator(mdates.AutoDateLocator(minticks=4, maxticks=8))
        ax_obj.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
        plt.setp(ax_obj.get_xticklabels(), rotation=30, ha='right')

    def plot_analysis(self, ic_df, group_returns, cumulative_returns, monthly_ic, cumulative_ls=None):
        fig = plt.figure(figsize=(20, 16))
        fmt = self._format_date_axis
        ...
        ax.set_title('Cumulative Returns', fontweight='bold'); ax.legend(loc='upper left'); ax.grid(True, alpha=0.3)
        fmt(ax)

        if cumulative_ls is not None:
            ax = plt.subplot(3, 3, 4)
            ax.plot(cumulative_ls.index, cumulative_ls, linewidth=3, color='purple')
        ...
        m60, s60 = roll.mean(), roll.std()
        ax.plot(ic_df.index, m60, label='60D Mean', linewidth=2)
        ax.fill_between(ic_df.index, m60 - s60, m60 + s60, alpha=0.3, label='+/-1std')
        ...
        ax.set_title('Annual IC', fontweight='bold'); ax.grid(True, alpha=0.3)
